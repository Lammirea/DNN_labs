...
# в качестве донора возьмем преобученную на ImageNet наборе сеть Inception V3
...
with torch.no_grad(): # отключим вычисление граиентов, т.к. будем делать только прямой проход
    for images, labels in test_loader:
        ...
        pred = model(images) # делаем предсказание по пакету
...
torch.save(model.state_dict(), 'CnNet.ckpt')